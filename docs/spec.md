...
```
Todos exceto `POST /api/auth` e `POST /api/clients` exigem `Authorization: Bearer {token}`. Nos endpoints `/api/clients/{id}/...`, `{id}` só é aceito se for o próprio cliente autenticado — qualquer outro id responde 404 (não 403, pra não confirmar que o id existe; ver [spec.md §6](spec.md#6-decisões-técnicas-e-trade-offs-pra-citar-na-entrevista)).
...
PATCH  /api/messages/{id}/status              → marca mensagem como DELIVERED/READ manualmente
```
Esses seis endpoints extras não têm contrato TypeScript prévio em `fullstack.md` (são além do desafio original) — o contrato vivo e definitivo pra todos os 12 endpoints é o Swagger (`/swagger-ui.html`, TASK-20), não este documento. Manter os DTOs duplicados aqui só criaria mais um lugar pra ficar desatualizado a cada mudança de campo.
...
- **Conversão de plano — residual tratado de forma assimétrica por direção.** A primeira versão de `ClientService.convertPlan` registrava o residual do plano anterior como um `CREDIT` genérico nas duas direções, sem de fato somá-lo ao novo saldo/limite — o extrato dizia "crédito de X" mas esse valor nunca aparecia em lugar nenhum, uma inconsistência encontrada revisando o próprio extrato. A regra corrigida trata as duas direções como coisas diferentes, porque são: `PREPAID→POSTPAID` carrega dinheiro real do cliente (o saldo pré-pago), então o residual é somado ao valor informado e vira **um único `CREDIT`** ("Conversão de plano para X"). `POSTPAID→PREPAID` é o inverso — o limite mensal não usado nunca foi saldo do cliente, então em vez de descartá-lo em silêncio (ou fingir que virou crédito), ele é registrado como **`DEBIT` explícito** ("Limite mensal não utilizado zerado...") e o valor inicial do novo saldo pré-pago entra à parte como **`CREDIT`** ("Saldo inicial da conversão...") — os dois lançamentos ficam visíveis no histórico, e o saldo final é só o valor informado (sem somar o residual descartado). `convertPlan` delega cada direção pra um método próprio (`convertToPrepaid`/`convertToPostpaid`) mais um helper `recordIfPositive` compartilhado, em vez de condicionais aninhadas cobrindo os dois casos no mesmo método.
- **Isolamento entre clientes nos endpoints de administração (`/api/clients/{id}/...`).** `GET /{id}`, `POST /{id}/credit`, `POST /{id}/limit`, `POST /{id}/plan` e `GET /{id}/transactions` inicialmente confiavam no `{id}` da URL sem checar se batia com o `clientId` da sessão autenticada — diferente de `ConversationService.assertOwnership`, que já existia pra conversas/mensagens. Na prática, qualquer cliente autenticado conseguia ler ou alterar saldo/limite/plano de **qualquer outro cliente** só sabendo o UUID (achado numa rodada de QA end-to-end via API + navegador). Corrigido com `ClientController.assertSelf(id)`, chamado no início dos 5 endpoints, lançando `ClientNotFoundException` (404) quando `id` não é o do próprio cliente — mesmo padrão "não confirma existência" já usado pra conversas de outro cliente.
